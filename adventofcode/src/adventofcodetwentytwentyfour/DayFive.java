	...
	public static void main(String[] args) {
		...
		for(int i=0; i <input2.length;i++) {
			...
			for(int j=0;j<input2[i].length;j++) {
				for(int r=j+1;r<input2[i].length-j;r++) {
					if(input.indexOf(input2[i][j]+"|"+input2[i][r]) == -1) {
						inOrder=false;
						break;
					}
				}
			...
			if(inOrder) {
				count.add(input2[i][(int)(input2[i].length/2)]);
			}
			else {
		...
		int[] counter = new int[count.size()];
		for(int i=0;i<count.size();i++) {
			counter[i]=count.get(i);
		}
		
		int[][] test = new int[input3.size()][];
		for(int i =0;i<input3.size();i++) {
			test[i] = Util.Strings.strToIntarr(input3.get(i).toString());
		}
		
//		for(int m=10;m<100;m++) {
//			if(Util.Strings.stringCounter(input, ""+m)!=0) {
//				System.out.print(m+",");
//			}
//		}
//		System.out.println();
//		int[] preCalc = {11,13,14,16,17,18,21,22,23,24,25,26,28,29,32,33,34,35,36,44,45,48,51,53,59,62,65,66,67,71,74,76,77,78,79,83,84,85,86,88,89,91,92,93,94,95,97,98,99};
//		int[] oldCalc = new int[preCalc.length];
//		int lp=0;
//		while(Arrays.equals(preCalc, oldCalc) != true) {
//			oldCalc=preCalc.clone();
//			for(int i=0; i <test.length;i++) {
//				for(int j=0;j<preCalc.length;j++) {
//					for(int r=j+1;r<preCalc.length-j;r++) {
//						if(input.indexOf(preCalc[r]+"|"+preCalc[j]) != -1 || input.indexOf(preCalc[j]+"|"+preCalc[r]) == -1) {
//							int temp=preCalc[j];
//							preCalc[j] = preCalc[r];
//							preCalc[r] = temp;
//						}
...
//				}
//			}
//			if(lp%(test.length*test.length)==0) {
//				for(int i=0;i<preCalc.length;i++) {
//					System.out.print(preCalc[i]+",");
//				}
//				System.out.println();
//				for(int i=0;i<oldCalc.length;i++) {
//					System.out.print(oldCalc[i]+",");
//				}
//				System.out.println();
//				System.out.println();
//			}
...
//		}
		...
		for(int i=0; i <test.length;i++) {
				for(int j=0;j<test[i].length;j++) {
					for(int r=0;r<test[i].length;r++) {
						//Util.Strings.stringCounter(input, test[i][r]+"|"+test[i][j]) == 1
						if(input.indexOf(test[i][r]+"|"+test[i][j]) != -1) {
							System.out.println(input.substring(input.indexOf(test[i][r]+"|"+test[i][j]), input.indexOf(test[i][r]+"|"+test[i][j])+5));
							int temp=test[i][j];
							test[i][j] = test[i][r];
							test[i][r] = temp;
							for(int m=0;m<test[i].length;m++) {
								System.out.print(test[i][m]+",");
							}
							System.out.println();
							System.out.println();
		...
		int temper = 0;
		for(int i=0;i<test.length;i++) {
				temper += test[i][(int)(test[i].length/2)];
		}
